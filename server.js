...
app.post('/add', (req, res) => {
    const { name, email, department, salary, mobile_no } = req.body;
    ...
    const sql = 'INSERT INTO employees (name, email, department, salary, mobile_no) VALUES (?, ?, ?, ?, ?)';
    const params = [name, email || null, department, parseFloat(salary) || 0, mobile_no || null];
...
app.post('/update/:id', (req, res) => {
    ...
    const sql = 'UPDATE employees SET name = ?, email = ?, department = ?, salary = ?, mobile_no = ? WHERE id = ?';
    const params = [name, email || null, department, parseFloat(salary) || 0, mobile_no || null, id];
