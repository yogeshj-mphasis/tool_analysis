...
const DB_SOURCE = process.env.DB_SOURCE || (process.env.NODE_ENV === 'test' ? ':memory:' : 'employee.sqlite');
...
let db = new sqlite3.Database(DB_SOURCE, (err) => {
    if (err) {
        ...
        // Create an 'employees' table if it doesn't exist, then ensure email column exists
        db.serialize(() => {
